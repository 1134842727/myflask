...
from views.base_view import list_model,delete_model,create_model,patch_model,method_not_surpport_model,list_model_simple
from utils.pyjwt import assert_jwt, set_jwt
from utils.file_manager import file_upload,file_delete
#部署前记得修改自己的盐
salt ='linkai4836'
...
def certification(func):
    ...
    def main_func(*args, **kwargs):
        ...
        users = list_model_simple(User,{'username':username})
        if users.count() == 0:
            return 'token无效'
        user = users[0]
        if user.password == password:
            roles = list_model_simple(Role,{"id":user.role_id})
            if roles.count() == 0:
                return 'token无效'
...
def user(username,role_name):
    if request.method == 'GET':
        ...
        if role_name != 'super_admin':
            if user.json.get('data')[0].get('username') != username:
                return '非超管只能删除自己！'  
        img_id = user.json.get('data')[0].get('img')
        file_path = Img.query.filter_by(id=img_id)[0].url
        file_delete(file_path)
        delete_model(User,db,request.args.to_dict())
        return delete_model(Img,db,{'id':img_id})
    else:
